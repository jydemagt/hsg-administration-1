...
function ensure_schema_updates(PDO $pdo): void {
    ...
    if(db_table_exists($pdo,'lager_admins')){
        ...
        $superCount = (int)$pdo->query('SELECT COUNT(*) FROM lager_admins WHERE is_superadmin=1')->fetchColumn();
        if($superCount === 0){
    ...
    try{$pdo->exec('CREATE INDEX idx_res_status_prod_loc ON lager_reservations(status,product_id,location_id)');}catch(Throwable $e){}
    try{$pdo->exec('CREATE INDEX idx_stock_prod_loc ON lager_stock(product_id,location_id)');}catch(Throwable $e){}
