...
final class SimplePdf {
 ...
 private function imgObj(string $path): ?array {
  ...
  // Handle PNG images with 8-bit Alpha channel using PDF /SMask Soft Mask
  if($info[2]===IMAGETYPE_PNG && function_exists('imagecreatefrompng') && function_exists('gzcompress')){
   ...
   if($src){
    ...
    for($y=0;$y<$h;$y++){
     for($x=0;$x<$w;$x++){
      ...
      $b=$rgba&0xFF;
      $a=($rgba>>24)&0x7F;
      // Convert 7-bit GD alpha (0=opaque, 127=transparent) to PDF 8-bit alpha (255=opaque, 0=transparent)
      $alphaByte=(int)round((127 - $a) * 255 / 127);
      $rgbData.=chr($r).chr($g).chr($b);
      $alphaData.=chr($alphaByte);
     }
    }
    imagedestroy($src);
    $rgbStream=gzcompress($rgbData);
    $alphaStream=gzcompress($alphaData);
 ...
 public function output(string $filename): void {
  ...
  foreach($this->pages as $idx=>$page){
      ...
      foreach($page['images'] as $name=>$path){
          $img=$this->imgObj($path);
          if($img){
              $imgObjCount += !empty($img['is_png_smask']) ? 2 : 1;
          }
      }
      $nextObj+=$imgObjCount+1; // images + contentObj
  ...
  foreach($this->pages as $idx=>$page){
      ...
      foreach($page['images'] as $name=>$path){
          ...
          if(!empty($img['is_png_smask'])){
              $smaskObj=$next++;
              $objs[$smaskObj]='<< /Type /XObject /Subtype /Image /Width '.$img['w'].' /Height '.$img['h'].' /ColorSpace /DeviceGray /BitsPerComponent 8 /Filter /FlateDecode /Length '.strlen($img['alphaStream']).' >>' . "\nstream\n".$img['alphaStream']."\nendstream";
              $imgObj=$next++;
              $dict='<< /Type /XObject /Subtype /Image /Width '.$img['w'].' /Height '.$img['h'].' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /FlateDecode /SMask '.$smaskObj.' 0 R /Length '.strlen($img['rgbStream']).' >>';
              $objs[$imgObj]=$dict."\nstream\n".$img['rgbStream']."\nendstream";
              $xobjs[]='/'.$name.' '.$imgObj.' 0 R';
          }else{
              $obj=$next++;
              $objs[$obj]=$img['dict']."\nstream\n".$img['stream']."\nendstream";
              $xobjs[]='/'.$name.' '.$obj.' 0 R';
