...
<div class="table-wrap mobile-hide"><table><thead><tr><th>Produkt</th><?php foreach($locs as $l):?><th><?=h($l['name'])?></th><?php endforeach;?><th>Fysisk</th><th>Reserveret</th><th>Disponibelt</th><th style="text-align:right;">Handling</th></tr></thead><tbody><?php foreach($rows as $p):?><tr><td><button type="button" class="product-row product-detail-trigger product-row-button" data-product-id="<?=$p['id']?>" aria-label="Vis detaljer for <?=h($p['name'])?>"><img class="product-thumb" src="<?=h(product_image_url(($p['image_approval_status']??'')==='approved'?$p['image_path']:null))?>" alt="<?=h($p['name'])?>"><div><strong><?=h($p['name'])?></strong><br><span class="muted"><?=h($p['sku'])?><?=!empty($p['brand_name'])?' · '.h($p['brand_name']):''?></span></div></button></td><?php $map=[];foreach($stockBy[$p['id']]??[] as $x)$map[$x['id']]=$x;foreach($locs as $l):$x=$map[$l['id']]??['available'=>0];?><td><?=intval($x['available'])?></td><?php endforeach;?><td><?=$p['physical']?></td><td><?=$p['reserved']?></td><td class="available"><span class="badge green" style="font-size:14px; padding:4px 10px;"><?=$p['available']?> stk.</span></td><td style="text-align:right;"><?php if(can('reservations.create')):?><button type="button" class="open-reserve" data-product-id="<?=$p['id']?>">Reservér</button><?php endif;?></td></tr><?php endforeach;?>
<?php if(!$rows):?>
