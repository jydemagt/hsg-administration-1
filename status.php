...
<div class="table-wrap mobile-hide"><table class="status-table"><thead><tr><th class="status-product-col">Produkt</th><?php foreach($locs as $l):?><th class="status-loc-col"><?=h($l['name'])?></th><?php endforeach;?><th class="status-num-col">Fysisk</th><th class="status-num-col">Reserveret</th><th class="status-avail-col">Disponibelt</th><th class="status-action-col">Handling</th></tr></thead><tbody><?php foreach($rows as $p):?><tr><td class="status-product-col"><button type="button" class="product-row product-detail-trigger product-row-button" data-product-id="<?=$p['id']?>" aria-label="Vis detaljer for <?=h($p['name'])?>"><div class="product-thumb-frame"><img class="product-thumb" src="<?=h(product_image_url(($p['image_approval_status']??'')==='approved'?$p['image_path']:null))?>" alt="<?=h($p['name'])?>"></div><div class="product-info"><div class="product-title"><?=h($p['name'])?></div><div class="product-meta"><?=h($p['sku'])?><?=!empty($p['brand_name'])?' · '.h($p['brand_name']):''?></div></div></button></td><?php $map=[];foreach($stockBy[$p['id']]??[] as $x)$map[$x['id']]=$x;foreach($locs as $l):$x=$map[$l['id']]??['available'=>0];?><td class="status-loc-col"><?=intval($x['available'])?></td><?php endforeach;?><td class="status-num-col"><?=$p['physical']?></td><td class="status-num-col"><?=$p['reserved']?></td><td class="status-avail-col"><span class="badge green"><?=$p['available']?> stk.</span></td><td class="status-action-col"><?php if(can('reservations.create')):?><button type="button" class="open-reserve" data-product-id="<?=$p['id']?>">Reservér</button><?php endif;?></td></tr><?php endforeach;?>
<?php if(!$rows):?>
